...
eval_args_as_list :: proc(ctx: ^Eval_Context, args: []compiler.Arg) -> Expr_Result {
	...
	// where the value limits have to be applied: the splice form is how a list is doubled
	// (values.MAX_LIST_LEN), `{x}` is how one is nested (values.MAX_VALUE_DEPTH), and `{x, x}`
	// is how a shared sublist's expanded size is doubled (values.MAX_VALUE_SIZE).
	if values.too_big(result) {
